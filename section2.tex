...
Les modèles graphiques de sécurité, tels que les arbres d'attaque \cite{schneier1999attacktrees} ou les graphes d'attaque \cite{phillips1998graph}, sont largement utilisés pour représenter et analyser les scénarios d'attaque contre des systèmes informatiques. 
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{Exemples d’arbres d’attaque.}
  \label{fig:attack-trees}
\end{figure}
...
La \cref{fig:attack-trees} présente un exemple d’arbre d’attaque action-based (à gauche), et state-based (à droite). 
\newpage
...
% TODO: Parler des feuilles plutôt que des traces, parler des choix de sémantique (ex : weak SAND vs strong SAND)
% TODO: Parler des variantes (une feuille = une action, ou n'importe quelle séquence contenant cette action)

\begin{itemize}
  \item \textbf{Basé sur les actions (Action-based)} : où les feuilles de l'arbre sont étiquetées par des actions. 
    \begin{itemize}
      \item Dans \cite{jhawar2015attack}, une feuille ne reconnaît que la trace composée uniquement de l'action $a$.
      % TODO: Il manque une référence ici
      \item Dans INSÉRER CITATION, une feuille reconnaît toutes les traces contenant au moins l'action $a$ à n'importe quelle position.
    \end{itemize}
  \item \textbf{Basé sur les états (State-based)} : Utilisée dans \cite{audinot2017correct, pinchinat2020library}, où les feulles de l'arbre sont un couple \ag{\atinit}{\atfinal} avec $\atinit, \atfinal \in \prop$, représentant respectivement les conditions initiales et finales du système. Dans certains cas, $\atinit$ peut être omis si aucune condition initiale spécifique n'est requise, ou si elle est triviale.
...
\end{figure}
Dans \cite{sheyner2002automated} un graphe d'attaque (AG) est défini ainsi :

\begin{definition}[Graphe d'attaque]
  Un graphe d'attaque est un tuple $(S, \rightarrow, S_0, S_s)$, où $S$ est un ensemble de nœuds représentant les états du système, $\rightarrow \subseteq S \times S$ est une relation de transition, $S_0 \subseteq S$ correspond à l’ensemble des états initiaux, et $S_s \subseteq S$ désigne l’ensemble des états cibles que l’attaquant cherche à atteindre.
\end{definition}

La \cref{fig:attack-graph-example} illustre un exemple simple de graphe d’attaque, qui admet les modélisé les attaques dont le but est d’atteindre l’état $s_7$ à partir de l’état initial $s_0$. Des états intermédiaires sont inclus pour représenter les étapes clés de l’attaque.

Une autre définition proposée par \cite{pinchinat2014towards} impose que les transitions soient étiquetées par des actions de l’attaquant :

\begin{definition}[Graphe d'attaque étiqueté]
  Un graphe d'attaque étiqueté est un tuple $(S, \delta, S_0, S_s)$, où $S$, $S_0$ et $S_s$ sont définis comme pour un graphe d'attaque classique, et $\delta : S \times \actions \rightarrow S$ est une fonction de transition étiquetée par des éléments de $\actions$, représentant les actions de l’attaquant.
\end{definition}
...
\begin{enumerate}
  \item Un graphe d’attaque $G = (S, \delta, S_0, S_s)$, où toutes les transitions de $G$ sont étiquetées par des actions issues de $\actions$ ;
  \item Une hiérarchie d'actions de haut niveau $\mathbb{H} = (\{\mathcal{H}_k) \}_{0 \leq k \leq K}, \mathcal{R})$, où :
    \begin{itemize}
      \item $\mathcal{H}_0 = \actions$ est l'ensemble des actions élémentaires de l'attaquant ;
...
% TODO : Peut-être donner une traduction d'AT en AG
